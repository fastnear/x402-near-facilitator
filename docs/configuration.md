...
| --- | --- | --- |
| `DATABASE_URL_FILE` | `database-url` | Neon service-role URL for this environment |
| `DATABASE_DIRECT_URL_FILE` | `database-direct-url` | Direct Neon URL for session leadership; may equal the application URL only when it is already direct |
...
| `OTEL_EXPORTER_OTLP_HEADERS_FILE` | `otel-headers` | Honeycomb OTLP authorization header |
...
The non-secret OTLP endpoint is `https://api.honeycomb.io`. Set resource
attributes for `service.name=x402-near-facilitator`,
`deployment.environment.name`, and `service.version`. Never put a dataset name
or API key in source-controlled examples if it identifies a private
environment.
...
| Backup RPC | `rpc.testnet.near.org` | `rpc.mainnet.near.org` |
...
Create independent Neon projects or branches and databases for mainnet and
testnet. Each environment has:
...
Use Neon pooled URLs for normal service traffic and a direct URL for the
session-leadership connection. The service permits the direct URL to fall back
to the application URL for local PostgreSQL, but production must provide the
explicit direct credential. Do not reuse another FastNEAR database or role.
